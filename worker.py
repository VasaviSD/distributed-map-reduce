...
class Worker:
    ...
    def execute_map_task(self, task_id, filepath, n_reduce):
            ...
            for region in range(n_reduce):
                ...
                with open(intermediate_file, 'w') as f:
                    for word, count in word_counts.items():
                        if partition_function(word, n_reduce) == region:
    ...
    def execute_reduce_task(self, task_id, region, intermediate_files):
        ...
        while self.running:
            try:
                ...
                if task_type == "map":
                    ...
                    file_locations = self.execute_map_task(task_id, filepath, n_reduce)
                    self.coordinator.root.submit_map_result(self.worker_id, task_id, file_locations)
                    
                elif task_type == "reduce":
                    region, intermediate_files = input_data
                    output_file = self.execute_reduce_task(task_id, region, intermediate_files)
                    self.coordinator.root.submit_reduce_result(self.worker_id, task_id, output_file)
